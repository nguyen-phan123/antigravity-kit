...
Ask the user for the Ticket Name/ID (e.g. `PROJ-123`) if not already provided.
If a ticket is identified, it MUST be prefixed to the commit message (e.g. `[PROJ-123] ...`).
...
git commit -m "[<TICKET-ID>] <type>(<scope>): <description>"
```
...
1. **Ask for Ticket ID** if not provided in the conversation context.
2. Show the user what will be committed (files and diff summary)
3. Suggest the commit type and scope
4. Propose a commit message (including `[Ticket-ID]` prefix)
